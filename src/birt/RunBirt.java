...
public byte[] runReport(ServletContext sc, HttpServletRequest req) throws ServletException{
		...
		try
		{
			...
			//Open report design
			design = birtReportEngine.openReportDesign( sc.getRealPath("/reports/") + "/" +  this.getReportName() );
			...
			if( req.getParameter("structid") != null ){
				task.setParameterValue("structid", Integer.valueOf(req.getParameter("structid")));
			}
			else {
			...
			// NO optionsPDF.setOutputFileName(PDFRenderOption.OUTPUT_FILE_NAME);
